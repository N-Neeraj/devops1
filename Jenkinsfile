pipeline {
  ...
  environment {
    ...
    SECURITY_GROUPS = 'sg-0e0da67de7e196a4e'  // <-- REPLACE with your SG id, e.g. sg-0abc1234
  }
  ...
  stages {
    ...
    stage('(Optional) Push to Docker Hub') {
      when { expression { return false } } // change expression to true if you want to push also to Docker Hub
      steps {
        ...
        withCredentials([usernamePassword(credentialsId: '<AWS_CREDENTIALS_ID>', usernameVariable: 'AWS_ACCESS_KEY_ID', passwordVariable: 'AWS_SECRET_ACCESS_KEY')]) {
          sh '''
            ...
            if [ "$SERVICE" = "ACTIVE" ]; then
              ...
              aws ecs create-service --cluster ${ECR_REPO} --service-name ${ECR_REPO} --task-definition ${ECR_REPO} --desired-count 1 --launch-type FARGATE --network-configuration "awsvpcConfiguration={subnets=[${SUBNETS}],securityGroups=[${SECURITY_GROUPS}],assignPublicIp=ENABLED}"
            fi
          '''
        }
      }
  ...
  post {
    success {
      ...
      // optional: send email with emailext plugin
    }
    failure {
